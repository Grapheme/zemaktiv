                ...
                <div class="done__house">
                    <div class="wrapper">
                        <div class="house__left">
                            <div style="{{ $hasImage ? 'background-image: url('.asset(Config::get('site.galleries_photo_public_dir').'/'.$build->photo->name).')' : '' }};" class="left__main-image"></div>
                    @if($hasGallery)
                        ...
                        @foreach($build->gallery->photos as $photo)
                            @if(File::exists(Config::get('site.galleries_photo_dir').'/'.$photo->name))
                                <a href="#" class="images__item">
                                    <span style="background-image: url({{ asset(Config::get('site.galleries_photo_public_dir').'/'.$photo->name) }});" class="item__image"></span>
                                </a>
                            @endif
                        @endforeach
                            </div>
                        ...
                        <div class="house__right">
                            ...
                            <div class="right__info">
                                <div class="info__block">
                                    ...
                                    <div class="block__right"><span>{{ number_format($build->area, 2, '.', ' ') }} кв.м</span></div>
                                </div>
                                <div class="info__block">
                                ...
                                <div class="info__block">
                                    ...
                                    <div class="block__right"><span>{{ $build->land_area }} {{ Lang::choice('сотка|сотки|соток', $build->land_area) }}</span></div>
                                </div>
                                <div class="info__block">
                                ...
                                <div class="info__block">
                                    ...
                                    <div class="block__right"><span>{{ number_format($build->price, 2, '.', ' ') }} руб</span></div>
                                </div>
                            </div>
                            <div class="right__btns">
                                <a href="{{ pageurl('gen-plan').'?land='.$build->id }}" class="us-btn btn-white"><span>Посмотреть на генплане</span></a>
